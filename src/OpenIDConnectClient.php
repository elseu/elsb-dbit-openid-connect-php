...
use OpenIDConnect\Constants\OpenIdConnectClientConstants;
use OpenIDConnect\ValueObject\State;
...
class OpenIDConnectClient
{
    ...
    /**
     * @var mixed stores the token response
     */
    ...
    /**
     * @var array|object holds verified jwt claims
     */
     ...
     */
    protected bool $httpUpgradeInsecureRequests = true;
     ...
     */
    private array $pkceAlgs = ['S256' => 'sha256', 'plain' => false];
    ...
    public function addAdditionalState(string $key, $value): void
    {
        $this->additionalState[$key] = $value;
    }

    /**
     * @param array $additionalState
     */
    public function setAdditionalState(array $additionalState): void
    {
...
//                }
        ...
        if ($this->allowImplicitFlow && isset($_REQUEST['id_token'])) {
            ...
            if ($this->canVerifySignatures()) {
                ...
                if (!$this->verifyJWTsignature($id_token)) {
                    throw new OpenIDConnectClientException ('Unable to verify signature: ' - $id_token);
                }
            } else {
                user_error('Warning: JWT signature verification unavailable.');
            }
    ...
    /**
     * @param array $claims - example: preferred_username, sub, etc.
     */
    public function addOptionalClaims($claims)
    {
        $this->optionalClaims = (array)$claims;
    }
    ...
    private function requestAuthorization()
    {
        ...
        if (count($this->scopes) > 0) {
            $auth_params = array_merge($auth_params,
                array('scope' => implode(' ', array_merge($this->scopes, array('openid')))));
        }

        if (count($this->optionalClaims) > 0) {
            $auth_params = array_merge($auth_params,
                array('claims' => json_encode($this->getOptionalClaims())));
        }
        ...
        if (!empty($ccm) && in_array($this->getCodeChallengeMethod(),
                $this->getProviderConfigValue('code_challenge_methods_supported'))) {
            $codeVerifier = bin2hex(random_bytes(64));
            $this->setCodeVerifier($codeVerifier);
            if (!empty($this->pkceAlgs[$this->getCodeChallengeMethod()])) {
                $codeChallenge = rtrim(strtr(base64_encode(hash($this->pkceAlgs[$this->getCodeChallengeMethod()],
                    $codeVerifier, true)), '+/', '-_'), '=');
            } else {
    ...
    protected function verifyJWTclaims($claims, $accessToken = null)
    {
        ...
        $isIssuerValidated = ($this->issuerValidator)($claims->iss);
        return ($isIssuerValidated
    ...
    private function buildEncodedState(): string
    {
        $state = [
            OpenIdConnectClientConstants::STATE_SIGNATURE => $this->generateRandString()
        ];

        if (!empty($this->additionalState)) {
    ...
    private function decodeState(?string $encodedState): State
    {
        ...
        unset($state[OpenIdConnectClientConstants::STATE_SIGNATURE]);
